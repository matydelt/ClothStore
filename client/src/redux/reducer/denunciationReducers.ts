...
import type { Action } from "../actions/denunciationActions";
import type { Publication, User } from "./stateTypes";
...
const DenunciationReducer: Reducer<DenunciationState, Action> = (
  state = { denunciations: [], loading: false },
  action
) => {
  switch (action.type) {
    ...
    case "DENUNCIATION_GET_SUCCESS":
      return {
        ...state,
        loading: false,
        denunciations: action.payload?.success,
      };
